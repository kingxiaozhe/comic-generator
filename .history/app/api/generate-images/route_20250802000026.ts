...
const aspectRatioToSize: Record<AspectRatioKey, string> = {
  "1:1": "1024x1024", // （1:1）
  "3:4": "864x1152", // （3:4）
  "4:3": "1152x864", // （4:3）
  "16:9": "1280x720", // （16:9）
  ...
  "3:2": "1248x832", // （3:2）
  "21:9": "1512x648", // （21:9）
  custom: "1024x1024", // 默认为1:1
};

export async function POST(request: NextRequest) {
  try {
    const { comicPanels, aspectRatio = "16:9" } = await request.json();
    ...
    const imageGenerationPromises = comicPanels.map(
      async (panel: ComicPanel, index: number) => {
        try {
          ...
          // 获取对应比例的尺寸
          const size =
            aspectRatioToSize[aspectRatio as AspectRatioKey] || "1024x1024";

          console.log(
            `生成场景 ${index + 1}, 使用尺寸: ${size}, 比例: ${aspectRatio}`
          );
