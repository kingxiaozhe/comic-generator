...
export async function POST(request: NextRequest) {
  try {
    const { content, count = 4, sceneNumber } = await request.json();

    if (!content || content.trim() === "") {
      return NextResponse.json({ error: "文章内容不能为空" }, { status: 400 });
    }

    // 构建提示词，要求创作完整的漫画故事或单个场景
    let prompt = "";

    if (sceneNumber) {
      // 单个场景的提示词
      prompt = `
你是一位专业的漫画脚本创作者。请根据以下内容，创作漫画故事的第${sceneNumber}个场景。
...
`;
    ...
    const comicPanels = scenes.map((content: string, index: number) => ({
      id: sceneNumber ? `panel-${sceneNumber}` : `panel-${index + 1}`,
      content: content,
